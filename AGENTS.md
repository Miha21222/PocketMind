...
- Tasks are local-first in browser storage, centered on `frontend/src/features/tasks/`. App settings also stay local-only in `localStorage`; the backend does not persist user settings and instead receives reminder-related snapshots on each synced task.
- Backend routes currently live under `/api/v1` and include `/auth/telegram`, `/sync/bootstrap`, `/sync/changes`, `/sync/tasks/{client_task_id}`, `/sync/batch`, and `/voice/transcribe`, plus `/health`.
...
- Telegram reminder actions are handled in `backend/app/bot/handlers/callbacks.py`, with done/snooze actions mutating task state and cleaning up reminder messages.
...
- Keep task behavior local-first and sync-oriented.
- Treat settings as client-owned local state unless the repo changes; do not reintroduce backend settings persistence casually.
